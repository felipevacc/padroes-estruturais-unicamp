...
class CobrancaServiceTest {
    ...
    void deveAplicarDescontoDeFidelidade() {
        ResultadoCobranca resultado = service.cobrar(pedido, FormaPagamento.BOLETO, true, false, false, false);
        assertEquals(950.0, resultado.getValorCobrado(), 0.001);
    }

    @Test
    void deveAplicarJurosDeParcelamento() {
        ResultadoCobranca resultado = service.cobrar(pedido, FormaPagamento.BOLETO, false, true, false, false);
        assertEquals(1029.9, resultado.getValorCobrado(), 0.001);
    }

    @Test
    void deveAplicarTaxaInternacional() {
        ResultadoCobranca resultado = service.cobrar(pedido, FormaPagamento.BOLETO, false, false, true, false);
        assertEquals(1050.0, resultado.getValorCobrado(), 0.001);
    }

    @Test
    void deveAplicarSeguro() {
        ResultadoCobranca resultado = service.cobrar(pedido, FormaPagamento.BOLETO, false, false, false, true);
        assertEquals(1004.90, resultado.getValorCobrado(), 0.001);
    }

    @Test
    void deveAplicarTodosOsAjustesNaOrdemDefinida() {
        ResultadoCobranca resultado = service.cobrar(pedido, FormaPagamento.BOLETO, true, true, true, true);
